@extends('layouts.app')
@section('title', __t('account.profile_meta_title', 'My Profile — ' . ($settings['store_name'] ?? 'MST Import and Export Sdn Bhd')))
...
<div class="page-header" style="padding-top:calc(75px + var(--space-6));background:linear-gradient(135deg, #091a36 0%, #0f274a 45%, #1e3a8a 100%);color:#ffffff;border-bottom:1px solid #1e3a8a;padding-bottom:var(--space-8);position:relative;overflow:hidden">
    ...
    <div class="container page-header-content" style="position:relative;z-index:2">
        <div class="breadcrumb" style="margin-bottom:4px">
            ...
            <span class="breadcrumb-sep" style="color:#60a5fa">›</span>
            <span style="font-weight:600;color:#ffffff">@t('account.nav_profile', 'Profile Settings')</span>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
            <div>
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                    @if($user->customer_group === 'trading')
                        <span style="background:rgba(56,189,248,0.18);border:1px solid rgba(186,230,253,0.35);padding:2px 9px;border-radius:999px;font-size:0.7rem;font-weight:700;color:#7dd3fc;text-transform:uppercase;letter-spacing:0.05em">
                            @t('account.trading_partner_badge', '🏢 B2B Trading Partner Profile')
                        </span>
                        <span style="color:#bae6fd;font-size:0.78rem">@t('account.trading_partner_tag', 'Commercial Wholesale & Container Supply')</span>
                    @elseif($user->customer_group === 'wholesale')
                        <span style="background:rgba(56,189,248,0.18);border:1px solid rgba(186,230,253,0.35);padding:2px 9px;border-radius:999px;font-size:0.7rem;font-weight:700;color:#7dd3fc;text-transform:uppercase;letter-spacing:0.05em">
                            @t('account.wholesale_partner_badge', '🏢 Wholesale Partner Profile')
                        </span>
                        <span style="color:#bae6fd;font-size:0.78rem">@t('account.wholesale_partner_tag', 'Bulk Carton & F&B Supply')</span>
                    @else
                        <span style="background:rgba(56,189,248,0.18);border:1px solid rgba(186,230,253,0.35);padding:2px 9px;border-radius:999px;font-size:0.7rem;font-weight:700;color:#7dd3fc;text-transform:uppercase;letter-spacing:0.05em">
                            @t('account.member_badge', '👤 Member Profile')
                        </span>
                        <span style="color:#bae6fd;font-size:0.78rem">@t('account.member_tag', 'Personal Account & Cold-Chain Delivery')</span>
                    @endif
                </div>
                <h1 class="page-title" style="color:#ffffff;font-family:var(--font-heading);font-size:clamp(1.5rem,3vw,1.95rem);margin-bottom:4px;letter-spacing:-0.02em">
                    @t('account.profile_page_title', 'Account Settings & Profile')
                </h1>
                <p class="page-subtitle" style="color:#e0f2fe;font-size:0.88rem;max-width:680px;line-height:1.4;margin:0">
                    @if($user->customer_group === 'trading')
                        @t('account.profile_subtitle_trading', 'Manage your company trading credentials, SSM registration, contact personnel, and delivery ports.')
                    @else
                        @t('account.profile_subtitle_member', 'Manage your personal information, contact credentials, and default shipping addresses.')
                    @endif
                </p>
            </div>
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
                <a href="{{ route('account.dashboard') }}" class="btn btn-secondary btn-sm" style="background:rgba(255,255,255,0.12);color:#ffffff;border:1px solid rgba(255,255,255,0.25);border-radius:10px;font-weight:600">
                    @t('account.back_to_dashboard', '← Back to Dashboard')
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="profile-hero-card">
            <div class="profile-hero-main">
                    ...
                    <div>
                        ...
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                            <span class="group-badge group-{{ $user->customer_group }}" style="font-size:0.75rem;padding:3px 10px;border-radius:6px">
                                {{ __t('account.tier_' . $user->customer_group, ucfirst($user->customer_group)) }} @t('account.tier_suffix', 'Account')
                            </span>
                            @if($user->isAdmin())
                                <span style="font-size:0.72rem;background:#fee2e2;color:#991b1b;padding:3px 8px;border-radius:6px;font-weight:700">@t('account.admin_role_badge', 'Admin')</span>
                            @endif
                            @if($user->isPending())
                                <span class="badge badge-warning" style="background:#fef3c7;color:#92400e;padding:3px 8px;border-radius:6px;font-size:0.72rem;font-weight:700">@t('account.approval_pending_badge', 'Approval Pending')</span>
                            @elseif($user->isApproved())
                                <span class="badge badge-success" style="background:#dcfce7;color:#15803d;padding:3px 8px;border-radius:6px;font-size:0.72rem;font-weight:700">@t('account.verified_approved_badge', '✓ Verified & Approved')</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="profile-hero-meta">
                    ...
                    <span>{{ __t('account.member_since', 'Member since :date', ['date' => $user->created_at->format('M Y')]) }}</span>
                </div>
            </div>
            ...
            <div class="profile-info-grid">
                ...
                <div class="profile-info-tile">
                    <div class="profile-info-tile-label">@t('account.registered_company_label', 'Registered Company')</div>
                    <div class="profile-info-tile-value">{{ $user->company_name }}</div>
                </div>
                @endif
                @if($user->company_reg_no)
                <div class="profile-info-tile">
                    <div class="profile-info-tile-label">@t('account.registration_no_label', 'Registration No / SSM')</div>
                    <div class="profile-info-tile-value">{{ $user->company_reg_no }}</div>
                </div>
                @endif
                @if($user->business_type)
                <div class="profile-info-tile">
                    <div class="profile-info-tile-label">@t('account.business_type_label', 'Business Type')</div>
            ...
            <form action="{{ route('account.profile.update') }}" method="POST">
                ...
                <div>
                    <div class="profile-section-header">
                        ...
                        <div class="profile-section-title">@t('account.section_personal_title', 'Personal & Contact Details')</div>
                    </div>
                    <div class="profile-section-sub">@t('account.section_personal_sub', 'Your personal identity and primary phone contact for order delivery notifications.')</div>

                    <div class="profile-grid-2">
                        <div class="form-group" style="margin-bottom:0">
                            <label class="form-label" style="font-weight:700">@t('account.field_fullname', 'Full Name') <span class="required" style="color:#ef4444">*</span></label>
                        ...
                        <div class="form-group" style="margin-bottom:0">
                            <div style="display:flex;align-items:center;justify-content:space-between">
                                <label class="form-label" style="font-weight:700">@t('account.field_email_login', 'Email Address (Login)')</label>
                                <span style="font-size:0.75rem;color:#64748b;font-weight:600">@t('account.readonly_badge', '🔒 Read-only')</span>
                            </div>
                            ...
                            <span class="text-xs text-muted" style="display:block;margin-top:4px;font-size:0.75rem">@t('account.email_readonly_note', 'Primary login email cannot be changed directly')</span>
                        </div>
                    </div>

                    <div class="form-group" style="margin-top:18px;margin-bottom:0">
                        <label class="form-label" style="font-weight:700">@t('account.field_phone_whatsapp', 'Phone Number (WhatsApp / Calls)') <span class="required" style="color:#ef4444">*</span></label>
                        ...
                        <span class="text-xs text-muted" style="display:block;margin-top:4px;font-size:0.75rem">@t('account.phone_driver_note', 'Used by drivers for shipping delivery confirmation and receipt SMS')</span>
                ...
                <div>
                    <div class="profile-section-header">
                        <div class="profile-section-icon">📍</div>
                        <div class="profile-section-title">@t('account.section_address_title', 'Default Delivery Address')</div>
                    </div>
                    <div class="profile-section-sub">@t('account.section_address_sub', 'Standard shipping destination applied automatically to your future seafood orders.')</div>

                    <div class="form-group" style="margin-bottom:18px">
                        <label class="form-label" style="font-weight:700">@t('account.field_street_address', 'Street Address') <span class="required" style="color:#ef4444">*</span></label>
                    ...
                    <div class="profile-grid-3">
                        <div class="form-group" style="margin-bottom:0">
                            <label class="form-label" style="font-weight:700">@t('account.field_city', 'City') <span class="required" style="color:#ef4444">*</span></label>
                        ...
                        <div class="form-group" style="margin-bottom:0">
                            <label class="form-label" style="font-weight:700">@t('account.field_state', 'State') <span class="required" style="color:#ef4444">*</span></label>
                        ...
                        <div class="form-group" style="margin-bottom:0">
                            <label class="form-label" style="font-weight:700">@t('account.field_postcode', 'Postcode') <span class="required" style="color:#ef4444">*</span></label>
                ...
                <div class="profile-actions">
                    ...
                    <button type="submit" class="btn btn-primary" style="padding:12px 32px;border-radius:10px;font-weight:700;box-shadow:0 4px 14px rgba(29, 78, 216, 0.25)">
                        ...
                        <span>@t('account.btn_save_changes', 'Save Profile Changes')</span>
                    </button>
                </div>
            </form>
        </div>
