@extends('layouts.app')
@section('title', 'My Dashboard — ' . ($settings['store_name'] ?? 'Mika Import and Export SDN Bhd'))
...
<div class="page-header" style="padding-top:calc(75px + var(--space-6));background:linear-gradient(135deg, #091a36 0%, #0f274a 45%, #1e3a8a 100%);color:#ffffff;border-bottom:1px solid #1e3a8a;padding-bottom:var(--space-8);position:relative;overflow:hidden">
    ...
    <div class="container page-header-content" style="position:relative;z-index:2">
        <div class="breadcrumb" style="margin-bottom:4px">
            ...
            <span class="breadcrumb-sep" style="color:#60a5fa">›</span>
            <span style="font-weight:600;color:#ffffff">Dashboard</span>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
            <div>
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                    ...
                    <span style="color:#bae6fd;font-size:0.78rem">Cold-Chain Seafood Management</span>
                </div>
                ...
                <p class="page-subtitle" style="color:#e0f2fe;font-size:0.88rem;max-width:680px;line-height:1.4;margin:0">
                    Manage your purchases, check fulfillment statuses, and access your profile preferences.
                </p>
            </div>
        ...
        <div class="dashboard-stats-grid">
            ...
            <div class="dashboard-stat-card">
                ...
                <div class="dashboard-stat-body">
                    ...
                    <div class="dashboard-stat-label">RFQs Submitted</div>
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="dashboard-card">
                <div class="dashboard-card-header">
                    ...
                    <a href="{{ route('account.orders') }}" class="btn btn-secondary btn-sm" style="border-radius:8px;font-size:0.8rem">
                        View All ({{ auth()->user()->orders()->count() }})
                    </a>
                </div>
                        ...
                        <div class="dashboard-order-row">
                            <div class="dashboard-order-left">
                                ...
                                <div class="dashboard-order-meta">
                                    {{ $order->created_at->format('d M Y, h:i A') }} · {{ $order->items->count() }} item{{ $order->items->count() > 1 ? 's' : '' }}
                                </div>
                            </div>
                    ...
                    <div class="dashboard-orders-mobile">
                        ...
                        <div class="dashboard-mobile-order-card">
                            ...
                            <div class="dashboard-mobile-order-foot">
                                <div>
                                    <div style="font-size:0.7rem;color:var(--gray-500);text-transform:uppercase;font-weight:700">Total</div>
                ...
                @else
                    <div class="empty-state" style="padding:48px 24px;text-align:center">
                        ...
                        <h3 style="font-size:1.15rem;font-weight:700;color:var(--seagreen-900);margin-bottom:6px">No Orders Yet</h3>
                        <p style="color:var(--gray-500);font-size:0.88rem;margin-bottom:20px">Browse our premium frozen seafood selection and place your first order.</p>
                        <a href="{{ route('shop.index') }}" class="btn btn-primary" style="border-radius:10px;padding:10px 22px">Start Shopping</a>
                    </div>
                @endif
            </div>
                    ...
                    <div class="dashboard-card-body">
                        <div class="dashboard-info-rows">
                            <div class="dashboard-info-row">
                                <span class="dashboard-info-label">Account Name</span>
                                <span class="dashboard-info-value">{{ auth()->user()->name }}</span>
                            </div>
                            <div class="dashboard-info-row">
                                <span class="dashboard-info-label">Email Address</span>
                                <span class="dashboard-info-value">{{ auth()->user()->email }}</span>
                            </div>
                            <div class="dashboard-info-row">
                                <span class="dashboard-info-label">Contact Phone</span>
                                <span class="dashboard-info-value">{{ auth()->user()->phone ?? '—' }}</span>
                            </div>
                            @if(auth()->user()->company_name)
                            <div class="dashboard-info-row">
                                <span class="dashboard-info-label">Company</span>
                                <span class="dashboard-info-value">{{ auth()->user()->company_name }}</span>
                            </div>
                            @endif
                            <div class="dashboard-info-row">
                                <span class="dashboard-info-label">Account Status</span>
